...
import {
  ChevronLeft, ArrowUpRight, ArrowDownLeft, Search,
...
function TransactionsContent() {
  ...
  const filtered = transactions.filter((tx) => {
    ...
    const matchBank   = filterBank === "all" || tx.bank === filterBank;
      ...
      <div className="grid grid-cols-2 gap-3 mb-5">
        <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="number-display text-base font-bold text-emerald-700">৳{totalIn.toLocaleString()}</p>
          </div>
        </div>
        <div className="bg-rose-50 border border-rose-100 rounded-xl p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="number-display text-base font-bold text-rose-700">৳{totalOut.toLocaleString()}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-5">
        ...
        <div className="relative">
          ...
          <select
            ...
            className="bg-white border border-slate-200 rounded-xl pl-8 pr-4 py-2.5 text-xs text-slate-700 focus:border-indigo-400 appearance-none"
          >
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
                      isCredit ? "bg-emerald-100 text-emerald-600" : "bg-rose-100 text-rose-500"
                    }`}>
